...
def _write_ozon_returns_sheet(spreadsheet, sheet_name, new_rows):
    try:
        worksheet = spreadsheet.worksheet(sheet_name)
    except gspread.WorksheetNotFound:
        worksheet = spreadsheet.add_worksheet(
            title=sheet_name, rows=1000, cols=len(OZON_RETURNS_HEADERS) + 1
        )
    ...
    for row in existing[1:]:
        row = list(row) + [""] * 20
        key = row[SHEET_DATA_START + IDX_OZON_RETURNS_KEY]
        if key:
            data_dict[key] = row[SHEET_DATA_START:SHEET_DATA_START + len(OZON_RETURNS_HEADERS)]

    for row in new_rows:
        data_dict[row[IDX_OZON_RETURNS_KEY]] = row

    sorted_rows = sorted(
        data_dict.values(),
        key=lambda r: r[IDX_OZON_RETURNS_DATE] if len(r) > IDX_OZON_RETURNS_DATE else "",
    )

    now = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=3)))
    service = ["Обновлен:", now.strftime("%Y-%m-%d"), now.strftime("%H:%M")]

    header_row = [""] + OZON_RETURNS_HEADERS
    all_rows = [header_row]
    for i, data_row in enumerate(sorted_rows):
        service_cell = service[i] if i < len(service) else ""
        all_rows.append([service_cell] + list(data_row))

    worksheet.resize(rows=max(len(all_rows), 1), cols=len(OZON_RETURNS_HEADERS) + 1)
    worksheet.update("A1", all_rows)
...
def write_ozon_returns_sheet(spreadsheet_id, sheet_name, new_rows):
    ...
    count = _write_ozon_returns_sheet(spreadsheet, sheet_name, new_rows)
